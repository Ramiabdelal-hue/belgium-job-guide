const translations = {
    ...
    fr: {
        ...
        dir: "ltr", tO: "✨ Services bientôt...", cta: "➕  Si vous souhaitez ajouter une boutique ou un service, veuillez cliquer ici  ➕"
    }
};
...
function changeLang(lang) {
    ...
    // تحديث اتجاه النص (مهم جداً للغة العربية)
    ...
    // ترجمة كروت قسم السيارات (شامل التاكسي)
    updateText('sub-c1', d.subC1); 
    updateText('sub-c2', d.subC2); 
    updateText('sub-c3-taxi', d.subC3); // تأكد أن الـ ID في HTML هو sub-c3-taxi
...
document.addEventListener('DOMContentLoaded', () => {
    changeLang('nl'); // لغة البداية

    // منطق التنقل بين الجداول (Toggle Logic)
    const setupToggle = (btnId, mainGridId, extraGridId) => {
        const btn = document.getElementById(btnId);
        if (btn) {
            btn.addEventListener('click', function() {
                document.getElementById(mainGridId).style.display = 'none';
                document.getElementById(extraGridId).style.display = 'block';
            });
        }
    };

    setupToggle('legal-toggle', 'main-newcomers-grid', 'extra-legal-grid');
    setupToggle('cars-toggle', 'main-cars-grid', 'extra-cars-grid');
});
...
function backToCarsMain() {
    ...
    const extraGrid = document.getElementById('extra-cars-grid');
